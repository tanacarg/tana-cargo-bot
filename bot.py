    ...
    for i, cargo in enumerate(open_cargo, 1):
        ...
        buttons.append([
            ...
            InlineKeyboardButton(
                ...
                callback_data=(
                    f"connect_{cargo_posts.index(cargo)}"
                )
            )
        ])
...
async def truck_capacity(update, context):
    ...
    await update.message.reply_text("4️⃣ መኪናው አሁን ያለበትን አድራሻ ይጻፉ።")
...
async def truck_phone(update, context):
    ...
    # Empty compatibility keys prevent old relay/display code from crashing.
    truck["from"] = ""
    truck["route"] = ""
...
async def find_truck(update, context):
    ...
    if not open_trucks:
        await update.message.reply_text(
            "🚛 በአሁኑ ጊዜ ክፍት የጭነት መኪና የለም።",
            reply_markup=main_menu()
        )
        return ConversationHandler.END

    text = "🚛 የተመዘገቡ የጭነት መኪናዎች፦\n\n"
    buttons = []
    for i, truck in enumerate(open_trucks, 1):
        original_index = truck_posts.index(truck)
        text += (
            f"🚛 መኪና #{truck.get('id', original_index + 1)}\n"
        ...
        buttons.append([InlineKeyboardButton(
            f"🤝 ግንኙነት ይጠይቁ #{truck.get('id', original_index + 1)}",
            callback_data=f"truckconnect_{original_index}"
        )])
...
async def error_handler(update, context):
    try:
        print("TANA CARGO error:", context.error)
    except Exception:
        pass
    try:
