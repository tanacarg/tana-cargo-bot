...
async def cargo_phone(
    ...
    # Optional notification to matching open trucks.
    for truck in truck_posts:
        if truck.get("status", "open") != "open":
            continue
        if vehicle_match(truck.get("type"), cargo.get("vehicle")) and route_match(
            truck.get("from", ""), truck.get("route", ""),
            cargo.get("from", ""), cargo.get("to", "")
        ):
            try:
...
async def find_cargo(
    ...
    open_cargo = [
        (i, cargo) for i, cargo in enumerate(cargo_posts)
    ...
    for index, cargo in open_cargo:
        ...
        buttons.append([InlineKeyboardButton(
            f"🤝 ግንኙነት ይጠይቁ #{post_id}",
            callback_data=f"connect_{index}"
        )])
...
async def find_truck(
    ...
    open_trucks = [
        (i, truck) for i, truck in enumerate(truck_posts)
        if truck.get("status", "open") == "open"
    ]

    if not open_trucks:
        await update.message.reply_text(
            "🚛 በአሁኑ ጊዜ ክፍት የሆነ የጭነት መኪና የለም።",
            reply_markup=main_menu()
        )
        return

    text = "🚛 የተመዘገቡ ክፍት የጭነት መኪኖች፦\n\n"
    buttons = []
    ...
    for index, truck in open_trucks:
        ...
        buttons.append([InlineKeyboardButton(
            f"🤝 ግንኙነት ይጠይቁ #{post_id}",
            callback_data=f"truckconnect_{index}"
        )])
...
async def profile(
    ...
    open_cargo = sum(1 for c in my_cargo if c.get("status", "open") == "open")
    completed_cargo = len(my_cargo) - open_cargo
    open_trucks = sum(1 for t in my_trucks if t.get("status", "open") == "open")
    completed_trucks = len(my_trucks) - open_trucks

    message += (
        f"\n🚚 የለጠፉት ጭነት፦ {len(my_cargo)}"
        f"\n   • ክፍት፦ {open_cargo}"
        f"\n   • የተጠናቀቀ፦ {completed_cargo}"
        f"\n\n🚛 የተመዘገቡ መኪኖች፦ {len(my_trucks)}"
        f"\n   • ክፍት፦ {open_trucks}"
        f"\n   • የተጠናቀቀ፦ {completed_trucks}"
    )
...
async def text_router(
    ...
    # A main-menu button always cancels stale user_data first.
...
async def error_handler(update, context):
    """Log unexpected Telegram errors without stopping the bot."""
