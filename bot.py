...
async def find_cargo(update, context):
    open_cargos = [c for c in cargo_posts if c.get("status", "open") == "open"]
    if not open_cargos:
        await update.message.reply_text(
            "📦 በአሁኑ ጊዜ ክፍት የሆነ ጭነት የለም።\n\n"
            "🔔 አዲስ ጭነት ሲገኝ ይመልከቱ።",
            reply_markup=main_menu()
        )
        return ConversationHandler.END
    await update.message.reply_text(
        "🔎 እባክዎ የፈለጉትን የጭነት መለያ ቁጥር ያስገቡ።\n\nምሳሌ፦ 1"
    )
    return FIND_CARGO_ID

async def find_cargo_by_id(update, context):
    try: idx = int(update.message.text.strip().replace("#", ""))
    except ValueError:
        await update.message.reply_text("❌ እባክዎ የጭነት መለያ ቁጥሩን ቁጥር ብቻ ያስገቡ።\nምሳሌ፦ 1")
        return FIND_CARGO_ID
    cargo = next((c for c in cargo_posts if c.get("id") == idx and c.get("status", "open") == "open"), None)
    if not cargo:
        await update.message.reply_text("❌ ይህ የጭነት መለያ አልተገኘም ወይም በስምምነት ተጠናቋል።\n\n🔎 እባክዎ ሌላ መለያ ይሞክሩ።", reply_markup=main_menu())
        return ConversationHandler.END
    post_index = cargo_posts.index(cargo)
    text = (f"📦 ጭነት #{cargo['id']}\n\n" f"📍 {cargo['from']} ➡️ {cargo['to']}\n" f"📦 አይነት፦ {cargo['type']}\n" f"🚛 መኪና፦ {cargo['vehicle']}\n" f"📊 {cargo['size_name']} ({cargo['size']}%)\n" f"⚖️ {cargo['weight']}\n" f"📅 {cargo['date']}\n" "💰 ዋጋ፦ በድርድር\n" "🔒 የባለቤቱ ስልክ ተደብቋል።")
    await update.message.reply_text(text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🤝 ግንኙነት ይጠይቁ", callback_data=f"connect_{post_index}")]]))
    return ConversationHandler.END
...
async def connection_request(
    ...
    if cargo.get("status", "open") != "open":
        await query.answer("⚠️ ይህ ጭነት በስምምነት ተጠናቋል።", show_alert=True)
        return
    ...
    relay_messages[relay_id] = {
        ...
        "receiver_id": cargo["user_id"],
        "text": (
            "🔔 አዲስ የግንኙነት ጥያቄ!\n\n"
            f"📦 ጭነት #{cargo['id']}\n"
            f"👤 ጠያቂ፦ {requester.full_name}\n\n"
            "📦 ጭነትዎን የሚጭን ደንበኛ ተገኝቷል።"
        ),
        "buttons": [[
    ...
    if ADMIN_USER_ID:
        try:
            await context.bot.send_message(chat_id=int(ADMIN_USER_ID), text=f"📨 TANA CARGO — አዲስ የግንኙነት ጥያቄ #{relay_id}\n\n{relay_messages[relay_id]['text']}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("➡️ ወደ ባለቤቱ ላክ", callback_data=f"relay_{relay_id}")]]))
        except Exception:
            pass
...
async def send_relay_to_admin(context, req_index, sender_id, text, buttons=None):
    ...
    relay_id = max(relay_messages.keys(), default=0) + 1
    relay_messages[relay_id] = {"req_index": req_index, "sender_id": sender_id, "receiver_id": receiver_id, "text": text, "buttons": buttons or []}
...
async def relay_forward(update, context):
    ...
    try:
        ...
        await context.bot.send_message(
            ...
            reply_markup=InlineKeyboardMarkup(item.get("buttons", [])) if item.get("buttons") else None
        )
...
async def truck_capacity(update, context):
    ...
    await update.message.reply_text("4️⃣ የመኪናውን አድራሻ / ያለበትን ቦታ ይጻፉ።")
...
async def find_truck(update, context):
    open_trucks = [t for t in truck_posts if t.get("status", "open") == "open"]
    if not open_trucks:
        await update.message.reply_text("🚛 በአሁኑ ጊዜ ክፍት የሆነ የጭነት መኪና የለም።", reply_markup=main_menu())
        return ConversationHandler.END
    await update.message.reply_text("🚛 እባክዎ የፈለጉትን የጭነት መኪና መለያ ቁጥር ያስገቡ።\n\nምሳሌ፦ 1")
    return FIND_TRUCK_ID

async def find_truck_by_id(update, context):
    try: idx = int(update.message.text.strip().replace("#", ""))
    except ValueError:
        await update.message.reply_text("❌ እባክዎ የመኪና መለያ ቁጥሩን ቁጥር ብቻ ያስገቡ።\nምሳሌ፦ 1")
        return FIND_TRUCK_ID
    truck = next((t for t in truck_posts if t.get("id") == idx and t.get("status", "open") == "open"), None)
    if not truck:
        await update.message.reply_text("❌ ይህ የመኪና መለያ አልተገኘም ወይም በስምምነት ተጠናቋል።", reply_markup=main_menu())
        return ConversationHandler.END
    post_index = truck_posts.index(truck)
    text = (f"🚛 መኪና #{truck['id']}\n\n" f"🔹 አይነት፦ {truck['type']}\n" f"⚖️ አቅም፦ {truck['capacity']}\n" f"📍 አድራሻ፦ {truck['address']}\n" "🔒 ታርጋና ስልክ ተደብቀዋል።")
    await update.message.reply_text(text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🤝 ግንኙነት ይጠይቁ", callback_data=f"truckconnect_{post_index}")]]))
    return ConversationHandler.END
...
async def truck_connection_request(
    ...
    if truck.get("status", "open") != "open":
        await query.answer("⚠️ ይህ መኪና በስምምነት ተጠናቋል።", show_alert=True)
        return
    ...
    relay_messages[relay_id] = {
        "req_index": len(connection_requests) - 1,
        "sender_id": requester.id,
        "receiver_id": truck["user_id"],
        "text": (
            "🔔 አዲስ የመኪና ግንኙነት ጥያቄ!\n\n"
            f"🚛 መኪና #{truck['id']}\n"
            f"👤 ጠያቂ፦ {requester.full_name}\n\n"
            "🚛 የእርስዎን መኪና የሚፈልግ ደንበኛ ተገኝቷል።"
        ),
        "buttons": [[
            InlineKeyboardButton("🤝 ግንኙነት", callback_data=f"accept_{len(connection_requests)-1}"),
            InlineKeyboardButton("❌ ውድቅ", callback_data=f"reject_{len(connection_requests)-1}")
    ...
    if ADMIN_USER_ID:
        try:
            await context.bot.send_message(chat_id=int(ADMIN_USER_ID), text=f"📨 TANA CARGO — አዲስ የመኪና ግንኙነት ጥያቄ #{relay_id}\n\n{relay_messages[relay_id]['text']}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("➡️ ወደ ባለቤቱ ላክ", callback_data=f"relay_{relay_id}")]]))
        except Exception:
            pass
...
async def profile(
    ...
    completed_cargo = sum(1 for c in my_cargo if c.get("status") == "completed")
    completed_trucks = sum(1 for t in my_trucks if t.get("status") == "completed")

    message += (
        f"\n🚚 የለጠፉት ጭነት፦ {len(my_cargo)}\n"
        f"🚛 የተመዘገቡ መኪኖች፦ {len(my_trucks)}\n"
        f"\n✅ በስምምነት የተጠናቀቁ ጭነቶች፦ {completed_cargo}\n"
        f"✅ በስምምነት የተጠናቀቁ መኪኖች፦ {completed_trucks}"
    )
...
async def error_handler(update, context):
    """Log unexpected Telegram bot errors without stopping the bot."""
    logger = logging.getLogger(__name__)
    logger.error(
        "TANA CARGO bot error",
...
def main():
    ...
    master_conversation = ConversationHandler(
        ...
        entry_points=[
            ...
            # ALL main-menu buttons must be ConversationHandler entry points.
            # This prevents Telegram from merely echoing a menu button while
            # another conversation state is active or after /start.
            MessageHandler(
                filters.Regex(r"^🔎 ጭነት መፈለግ$"),
                find_cargo,
            ),
            MessageHandler(
                filters.Regex(r"^🚛 መኪና መፈለግ$"),
                find_truck,
            ),
            MessageHandler(
                filters.Regex(r"^👤 የኔ መረጃ$"),
                profile,
            ),
            MessageHandler(
        ...
        states={
            ...
            TRUCK_ADDRESS: [
                MessageHandler(filters.Regex(r"^(🚚 ጭነት መለጠፍ|🔎 ጭነት መፈለግ|🚛 መኪና ማስመዝገብ|🚛 መኪና መፈለግ|👤 የኔ መረጃ|🤝 ግንኙነት ጥያቄዎች|📞 Support|ℹ️ About)$"), menu_interrupt),
            ...
            FIND_CARGO_ID: [
                MessageHandler(filters.Regex(r"^(🚚 ጭነት መለጠፍ|🔎 ጭነት መፈለግ|🚛 መኪና ማስመዝገብ|🚛 መኪና መፈለግ|👤 የኔ መረጃ|🤝 ግንኙነት ጥያቄዎች|📞 Support|ℹ️ About)$"), menu_interrupt),
                MessageHandler(filters.TEXT & ~filters.COMMAND, find_cargo_by_id),
            ],

            FIND_TRUCK_ID: [
                MessageHandler(filters.Regex(r"^(🚚 ጭነት መለጠፍ|🔎 ጭነት መፈለግ|🚛 መኪና ማስመዝገብ|🚛 መኪና መፈለግ|👤 የኔ መረጃ|🤝 ግንኙነት ጥያቄዎች|📞 Support|ℹ️ About)$"), menu_interrupt),
                MessageHandler(filters.TEXT & ~filters.COMMAND, find_truck_by_id),
            ],
