...
# STATES
# ==================================================
(CARGO_FROM, CARGO_TO, CARGO_TYPE, CARGO_VEHICLE, CARGO_SIZE, CARGO_WEIGHT, CARGO_DATE, CARGO_PHONE) = range(8)
(TRUCK_TYPE, TRUCK_PLATE, TRUCK_CAPACITY, TRUCK_ADDRESS, TRUCK_DRIVER, TRUCK_PHONE) = range(8, 14)
...
async def cargo_start(update, context):
    context.user_data.clear(); context.user_data["cargo"]={}
    await update.message.reply_text("🚚 ጭነት መለጠፍ\n\n1️⃣ ጭነቱ የሚነሳበትን ቦታ ይጻፉ።\nምሳሌ፦ ባህር ዳር")
    return CARGO_FROM


async def cargo_from(update, context):
    context.user_data["cargo"]["from"]=update.message.text.strip()
    await update.message.reply_text("2️⃣ የሚደርስበትን ቦታ ይጻፉ።"); return CARGO_TO


async def cargo_to(update, context):
    context.user_data["cargo"]["to"]=update.message.text.strip()
    await update.message.reply_text("3️⃣ የጭነቱን አይነት ይጻፉ።\nምሳሌ፦ ሲሚንቶ / እህል / ፍራሽ"); return CARGO_TYPE


async def cargo_type(update, context):
    context.user_data["cargo"]["type"]=update.message.text.strip()
    await update.message.reply_text("4️⃣ ለዚህ ጭነት የሚፈልጉትን የመኪና አይነት ይምረጡ።", reply_markup=vehicle_keyboard("cargo_vehicle")); return CARGO_VEHICLE


async def cargo_vehicle(update, context):
    query=update.callback_query; await query.answer(); value=query.data.replace("cargo_vehicle_", "", 1)
    if value=="other":
        context.user_data["cargo"]["vehicle_waiting"]=True; await query.edit_message_text("✍️ የሚፈልጉትን የመኪና አይነት ይጻፉ።"); return CARGO_VEHICLE
    context.user_data["cargo"]["vehicle"]=value
    await query.edit_message_text(f"🚛 የተመረጠው መኪና፦ {value}\n\n5️⃣ የጭነቱን መጠን ይምረጡ።", reply_markup=size_keyboard()); return CARGO_SIZE


async def cargo_vehicle_text(update, context):
    cargo=context.user_data.get("cargo", {})
    if not cargo.get("vehicle_waiting"): return CARGO_VEHICLE
    cargo["vehicle"]=update.message.text.strip(); cargo["vehicle_waiting"]=False
    await update.message.reply_text(f"🚛 የሚፈለገው መኪና፦ {cargo['vehicle']}\n\n5️⃣ የጭነቱን መጠን ይምረጡ።", reply_markup=size_keyboard()); return CARGO_SIZE


async def cargo_size(update, context):
    query=update.callback_query; await query.answer(); sizes={"size_100":(100,"ሙሉ ጭነት"),"size_50":(50,"ግማሽ ጭነት"),"size_25":(25,"እሩብ ጭነት")}
    if query.data not in sizes: return CARGO_SIZE
    size,name=sizes[query.data]; context.user_data["cargo"].update({"size":size,"size_name":name})
    await query.edit_message_text(f"📦 የተመረጠው፦ {name} ({size}%)\n\n6️⃣ የጭነቱን ክብደት ይጻፉ።\nምሳሌ፦ 10 ቶን"); return CARGO_WEIGHT


async def cargo_weight(update, context):
    context.user_data["cargo"]["weight"]=update.message.text.strip(); await update.message.reply_text("7️⃣ የሚጫንበትን ቀን ይጻፉ።\nምሳሌ፦ 25/09/2026"); return CARGO_DATE


async def cargo_date(update, context):
    context.user_data["cargo"]["date"]=update.message.text.strip(); await update.message.reply_text("8️⃣ ስልክ ቁጥር ይጻፉ።"); return CARGO_PHONE


async def cargo_phone(update, context):
    cargo=context.user_data["cargo"]; cargo["phone"]=update.message.text.strip(); cargo["user_id"]=update.effective_user.id; cargo["status"]="open"; cargo_posts.append(cargo.copy()); cid=len(cargo_posts)
    await update.message.reply_text("✅ ጭነትዎ በትክክል ተመዝግቧል!\n\n" f"🆔 ጭነት #{cid}\n📍 መነሻ፦ {cargo['from']}\n📍 መድረሻ፦ {cargo['to']}\n📦 አይነት፦ {cargo['type']}\n🚛 የሚፈለገው መኪና፦ {cargo['vehicle']}\n📊 መጠን፦ {cargo['size_name']} ({cargo['size']}%)\n⚖️ ክብደት፦ {cargo['weight']}\n📅 ቀን፦ {cargo['date']}\n💰 ዋጋ፦ በድርድር\n\n🔒 የስልክ ቁጥርዎ ለሌሎች አይታይም።", reply_markup=main_menu())
...
async def find_cargo(update, context):
    open_posts=[(i,c) for i,c in enumerate(cargo_posts) if c.get("status","open")!="completed"]
    if not open_posts:
        await update.message.reply_text("📦 በአሁኑ ጊዜ ክፍት የሆነ ጭነት የለም።", reply_markup=main_menu()); return ConversationHandler.END
    text="🔎 የተመዘገቡ ክፍት ጭነቶች፦\n\n"; buttons=[]
    for i,c in open_posts:
        text += f"📦 ጭነት #{i+1}\n📍 {c['from']} ➡️ {c['to']}\n📦 አይነት፦ {c['type']}\n🚛 መኪና፦ {c['vehicle']}\n📊 {c['size_name']} ({c['size']}%)\n⚖️ {c['weight']}\n📅 {c['date']}\n💰 ዋጋ፦ በድርድር\n🔒 የባለቤቱ ስልክ ተደብቋል።\n\n"
        buttons.append([InlineKeyboardButton(f"🤝 ግንኙነት ይጠይቁ #{i+1}", callback_data=f"connect_{i}")])
    await update.message.reply_text(text, reply_markup=InlineKeyboardMarkup(buttons)); return ConversationHandler.END
    ...
    try:
        ...
        await context.bot.send_message(
            ...
            text=(
                ...
                "እባክዎን መልእክቱ እንደደረስዎ በTANA CARGO ላይ 🤝 ግንኙነት የሚለውን ይጫኑና ይነጋገሩ።"
            ),
...
async def agree_price(
    ...
    if (
        ...
        if req.get("type") == "cargo":
            idx=req.get("cargo_index")
            if isinstance(idx,int) and 0 <= idx < len(cargo_posts): cargo_posts[idx]["status"]="completed"
        elif req.get("type") == "truck":
            idx=req.get("truck_index")
            if isinstance(idx,int) and 0 <= idx < len(truck_posts): truck_posts[idx]["status"]="completed"
...
async def truck_start(update, context):
    context.user_data.clear(); context.user_data["truck"]={}
    await update.message.reply_text("🚛 መኪና ማስመዝገብ\n\n1️⃣ የመኪናውን አይነት ይጻፉ።\nምሳሌ፦ Isuzu / ካሶኒ / ኦባማ"); return TRUCK_TYPE


async def truck_type(update, context):
    context.user_data["truck"]["type"]=update.message.text.strip(); await update.message.reply_text("2️⃣ የመኪናውን ታርጋ ይጻፉ።"); return TRUCK_PLATE


async def truck_plate(update, context):
    context.user_data["truck"]["plate"]=update.message.text.strip(); await update.message.reply_text("3️⃣ የመጫን አቅሙን ይጻፉ።\nምሳሌ፦ 30 ቶን"); return TRUCK_CAPACITY


async def truck_capacity(update, context):
    context.user_data["truck"]["capacity"]=update.message.text.strip(); await update.message.reply_text("4️⃣ የመኪናው የአሁኑን አድራሻ/ቦታ ይጻፉ።"); return TRUCK_ADDRESS
...
async def truck_address(update, context):
    context.user_data["truck"]["address"]=update.message.text.strip(); await update.message.reply_text("5️⃣ የሹፌሩን ስም ይጻፉ።"); return TRUCK_DRIVER


async def truck_driver(update, context):
    context.user_data["truck"]["driver"]=update.message.text.strip(); await update.message.reply_text("6️⃣ የስልክ ቁጥር ይጻፉ።"); return TRUCK_PHONE


async def truck_phone(update, context):
    truck=context.user_data["truck"]; truck["phone"]=update.message.text.strip(); truck["user_id"]=update.effective_user.id; truck["status"]="open"; truck_posts.append(truck.copy()); tid=len(truck_posts)
    await update.message.reply_text("✅ መኪናዎ በትክክል ተመዝግቧል!\n\n" f"🆔 መኪና #{tid}\n🚛 አይነት፦ {truck['type']}\n⚖️ አቅም፦ {truck['capacity']}\n📍 አድራሻ፦ {truck['address']}\n👨‍✈️ ሹፌር፦ {truck['driver']}\n\n🔒 ታርጋና ስልክ ቁጥር ለሌሎች አይታይም。", reply_markup=main_menu())
    return ConversationHandler.END
...
async def find_truck(update, context):
    open_posts=[(i,t) for i,t in enumerate(truck_posts) if t.get("status","open")!="completed"]
    if not open_posts:
        await update.message.reply_text("🚛 በአሁኑ ጊዜ ክፍት የሆነ የጭነት መኪና የለም።", reply_markup=main_menu()); return ConversationHandler.END
    text="🚛 የተመዘገቡ ክፍት መኪኖች፦\n\n"; buttons=[]
    for i,t in open_posts:
        text += f"🚛 መኪና #{i+1}\n🔹 አይነት፦ {t['type']}\n⚖️ አቅም፦ {t['capacity']}\n📍 አድራሻ፦ {t['address']}\n👨‍✈️ ሹፌር፦ {t['driver']}\n🔒 ታርጋና ስልክ ተደብቀዋል።\n\n"
        buttons.append([InlineKeyboardButton(f"🤝 ግንኙነት ይጠይቁ #{i+1}", callback_data=f"truckconnect_{i}")])
    await update.message.reply_text(text, reply_markup=InlineKeyboardMarkup(buttons)); return ConversationHandler.END
    ...
    try:
        ...
        await context.bot.send_message(
            ...
            text=(
                ...
                "እባክዎን በTANA CARGO ላይ 🤝 ግንኙነት የሚለውን ይጫኑና ይነጋገሩ።"
            ),
...
def main():
    ...
    master_conversation = ConversationHandler(
        ...
        states={
            ...
            CARGO_FROM: [
                MessageHandler(
                    filters.Regex(r"^(🚚 ጭነት መለጠፍ|🔎 ጭነት መፈለግ|🚛 መኪና ማስመዝገብ|🚛 መኪና መፈለግ|👤 የኔ መረጃ|🤝 ግንኙነት ጥያቄዎች|📞 Support|ℹ️ About)$"),
                    menu_interrupt,
                ),
                MessageHandler(
