...
async def cargo_phone(update, context):
    ...
    for truck in truck_posts:
        if truck.get("status", "open") == "completed":
            continue
        if truck.get("status", "open") != "completed" and vehicle_match(truck.get("type"), cargo.get("vehicle")):
            try:
...
async def find_cargo(update, context):
    open_cargos = [(i, c) for i, c in enumerate(cargo_posts) if c.get("status", "open") != "completed"]
    if not open_cargos:
        await update.message.reply_text(
            "ℹ️ አሁን የተመዘገበ ክፍት ጭነት የለም።",
            reply_markup=main_menu()
        )
        return ConversationHandler.END

    await update.message.reply_text("🔎 የተመዘገቡ ክፍት ጭነቶች፦")
    for index, cargo in open_cargos:
        text = (
            f"📦 ጭነት #{index + 1}\n\n"
            f"📍 መነሻ፦ {cargo.get('from', '')}\n"
            f"📍 መድረሻ፦ {cargo.get('to', '')}\n"
            f"📦 አይነት፦ {cargo.get('type', '')}\n"
            f"🚛 መኪና፦ {cargo.get('vehicle', '')}\n"
            f"📊 መጠን፦ {cargo.get('size_name', '')} ({cargo.get('size', '')}%)\n"
            f"⚖️ ክብደት፦ {cargo.get('weight', '')}\n"
            f"📅 ቀን፦ {cargo.get('date', '')}\n"
            "💰 ዋጋ፦ በድርድር\n"
            "🔒 የባለቤቱ ስልክ ተደብቋል።"
        )
        button = InlineKeyboardMarkup([[InlineKeyboardButton(
            "🤝 ግንኙነት ይጠይቁ", callback_data=f"connect_{index}"
        )]])
...
async def agree_price(
    ...
    if (
        ...
        # The cargo/truck listing is no longer available for new searches
        # once both parties have agreed on the final price.
        if req.get("type") == "cargo":
            post_index = req.get("cargo_index")
            if post_index is not None and 0 <= post_index < len(cargo_posts):
                cargo_posts[post_index]["status"] = "completed"
        elif req.get("type") == "truck":
            post_index = req.get("truck_index")
            if post_index is not None and 0 <= post_index < len(truck_posts):
                truck_posts[post_index]["status"] = "completed"
...
async def truck_capacity(update, context):
    ...
    await update.message.reply_text("4️⃣ የመኪናውን አድራሻ / የአሁኑን ቦታ ይጻፉ።")
    return TRUCK_ADDRESS

# Legacy route functions are kept only for compatibility; the current
# registration flow does NOT ask for starting point or route.
async def truck_from(update, context):
    return await truck_address(update, context)
...
async def find_truck(update, context):
    open_trucks = [(i, t) for i, t in enumerate(truck_posts) if t.get("status", "open") != "completed"]
    if not open_trucks:
        await update.message.reply_text(
            "ℹ️ አሁን የተመዘገበ ክፍት የጭነት መኪና የለም።",
            reply_markup=main_menu()
        )
        return ConversationHandler.END

    await update.message.reply_text("🚛 የተመዘገቡ ክፍት የጭነት መኪኖች፦")
    for index, truck in open_trucks:
        text = (
            f"🚛 መኪና #{index + 1}\n\n"
            f"🔹 አይነት፦ {truck.get('type', '')}\n"
            f"⚖️ አቅም፦ {truck.get('capacity', '')}\n"
            f"📍 አድራሻ፦ {truck.get('address', '')}\n"
            f"👨‍✈️ ሹፌር፦ {truck.get('driver', '')}\n"
            "🔒 ታርጋና ስልክ ተደብቀዋል።"
        )
        button = InlineKeyboardMarkup([[InlineKeyboardButton(
            "🤝 ግንኙነት ይጠይቁ", callback_data=f"truckconnect_{index}"
        )]])
        await update.message.reply_text(text, reply_markup=button)
...
async def profile(
    ...
    message += (
        ...
        "💡 TANA CARGO ስለ አገልግሎቱ፦\n"
        "ጭነት የሚያስጭኑ የጭነት ባለቤቶችን እና "
        "ለጭነት ጫኝ ሹፌሮችን በማገናኘት፣ "
        "ፈጣን መረጃ እንዲያገኙና ቀልጣፋ አገልግሎት "
        "እንዲያገኙ ለማድረግ የተዘጋጀ ነው።"
    )
...
async def menu_interrupt(update, context):
    # Always cancel the previous text-entry flow before opening a new menu item.
    text = update.message.text
    context.user_data.clear()
...
def main():
    ...
    master_conversation = ConversationHandler(
        ...
        entry_points=[
            ...
            # Every menu button can interrupt any active flow.
            MessageHandler(
