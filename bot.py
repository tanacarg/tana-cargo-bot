...
async def cargo_phone(update, context):
    ...
    cargo["post_id"] = len(cargo_posts) + 1
    cargo["status"] = "open"
...
async def find_cargo(
    ...
    if not any(c.get("status", "open") != "completed" for c in cargo_posts):
    ...
    for i, cargo in enumerate(
        ...
        if cargo.get("status", "open") == "completed":
            continue

        post_id = cargo.get("post_id", i)
        message += (
...
async def connection_request(
    ...
    admin_text = (
        "🤝 አዲስ የግንኙነት ጥያቄ\n\n"
        f"📌 አይነት፦ {request.get('type')}\n"
        f"👤 ጠያቂ፦ {requester.full_name}\n"
        "🔒 የግል ስልክ/ታርጋ መረጃ አይታይም።"
    )
    await send_relay_to_admin(context, len(connection_requests)-1, requester.id, admin_text)
...
async def agree_price(
    ...
    if (
        ...
        # Once both sides agree on the final price, the listing is no longer open.
...
async def truck_capacity(update, context):
    ...
    await update.message.reply_text("4️⃣ የመኪናውን አሁን ያለበትን አድራሻ/ቦታ ይጻፉ።\nምሳሌ፦ ባህር ዳር")
...
async def truck_phone(update, context):
    ...
    truck["post_id"] = len(truck_posts) + 1
    truck["status"] = "open"
...
async def find_truck(
    ...
    if not any(t.get("status", "open") != "completed" for t in truck_posts):

        await update.message.reply_text(

            "በአሁኑ ጊዜ እርሶ የሚፈልጉት አይነት የጭነት መኪና አልተገኘም።\n"
            "ትንሽ ቆይተው ይሞክሩ።",
...
async def profile(update, context):
    ...
    if not user:
        user = {"name": update.effective_user.full_name, "username": update.effective_user.username or "", "id": user_id}
        users[user_id] = user
    my_cargo=[c for c in cargo_posts if c.get("user_id")==user_id]
    my_trucks=[t for t in truck_posts if t.get("user_id")==user_id]
    ...
    message=(
        ...
        "ጭነት የሚያስጭኑ የጭነት ባለቤቶችን እና ለጭነት ጫኝ ሹፌሮችን በማገናኘት፣ ፈጣን መረጃ እንዲያገኙና ቀልጣፋ አገልግሎት እንዲያገኙ የሚያደርግ አገልግሎት።\n\n"
        f"🚚 የለጠፉት ጭነት፦ {len(my_cargo)}\n"
        f"✅ የተጠናቀቀ ጭነት፦ {completed_cargo}\n"
        f"🚛 የተመዘገቡ መኪኖች፦ {len(my_trucks)}\n"
        f"✅ የተጠናቀቁ መኪኖች፦ {completed_trucks}"
    )
...
async def support_start(update, context):
    ...
    await update.message.reply_text(
        ...
        "🤝 ለእርዳታ፣ ጥያቄ ወይም ችግር ካለዎት እባክዎ ያግኙን።\n\n"
        "ወይም መልዕክትዎን ከታች ይጻፉ።"
    )
...
async def error_handler(update, context):
    print(f"TANA CARGO error: {context.error!r}")
