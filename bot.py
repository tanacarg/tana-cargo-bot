...
async def find_cargo(update, context):
    open_cargo = [c for c in cargo_posts if c.get("status", "open") != "completed"]
    if not open_cargo:
        await update.message.reply_text("📦 በአሁኑ ጊዜ ክፍት ጭነት የለም።", reply_markup=main_menu())
        return
    message = "🔎 የተመዘገቡ ክፍት ጭነቶች፦\n\n"
    buttons=[]
    for i, cargo in enumerate(open_cargo, 1):
        real_index = cargo_posts.index(cargo)
        message += (f"📦 ጭነት #{real_index+1}\n"
                    ...
                    "💰 ዋጋ፦ በድርድር\n"
                    "🔒 የባለቤቱ ስልክ ተደብቋል።\n\n")
        buttons.append([InlineKeyboardButton(f"🤝 ግንኙነት ይጠይቁ #{real_index+1}", callback_data=f"connect_{real_index}")])
...
async def truck_capacity(update, context):
    ...
    await update.message.reply_text("4️⃣ የመኪናውን አድራሻ / አሁን ያለበትን ቦታ ይጻፉ።")
...
async def find_truck(update, context):
    open_trucks = [t for t in truck_posts if t.get("status", "open") != "completed"]
    if not open_trucks:
        await update.message.reply_text("🚛 በአሁኑ ጊዜ ክፍት የጭነት መኪና የለም።", reply_markup=main_menu())
        return
    text = "🚛 የተመዘገቡ ክፍት መኪኖች፦\n\n"
    buttons=[]
    for truck in open_trucks:
        real_index=truck_posts.index(truck)
        text += (f"🚛 መኪና #{real_index+1}\n"
                 f"🔹 አይነት፦ {truck['type']}\n"
                 f"⚖️ አቅም፦ {truck['capacity']}\n"
                 f"📍 አድራሻ፦ {truck['address']}\n"
                 f"👨‍✈️ ሹፌር፦ {truck['driver']}\n"
                 "🔒 ታርጋና ስልክ ተደብቀዋል።\n\n")
        buttons.append([InlineKeyboardButton(f"🤝 ግንኙነት ይጠይቁ #{real_index+1}", callback_data=f"truckconnect_{real_index}")])
...
MAIN_MENU_REGEX = r"^(🚚 ጭነት መለጠፍ|🔎 ጭነት መፈለግ|🚛 መኪና ማስመዝገብ|🚛 መኪና መፈለግ|👤 የኔ መረጃ|🤝 ግንኙነት ጥያቄዎች|📞 Support|ℹ️ About)$"
...
async def menu_interrupt(update, context):
    """Switch immediately to the newly selected main-menu action."""
    text = update.message.text.strip()
    context.user_data.clear()
    ...
    if text == "🔎 ጭነት መፈለግ":
        await find_cargo(update, context); return ConversationHandler.END
    if text == "🚛 መኪና መፈለግ":
        await find_truck(update, context); return ConversationHandler.END
    if text == "👤 የኔ መረጃ":
        await profile(update, context); return ConversationHandler.END
    if text == "🤝 ግንኙነት ጥያቄዎች":
        await show_connection_requests(update, context); return ConversationHandler.END
    if text == "ℹ️ About":
        await about(update, context); return ConversationHandler.END
    return ConversationHandler.END
...
def main():
    ...
    master_conversation = ConversationHandler(
        ...
        states={
            ...
            TRUCK_ADDRESS: [
                MessageHandler(filters.Regex(MAIN_MENU_REGEX), menu_interrupt),
